#!/usr/bin/env bash
# Install / reinstall the LaunchAgent that runs the mac-cpu-monitord backend
...
LABEL="com.maximofn.mac-cpu-monitord"
SRC="$(cd "$(dirname "$0")" && pwd)/${LABEL}.plist"
DST="$HOME/Library/LaunchAgents/${LABEL}.plist"
BIN="$(cd "$(dirname "$0")/../.." && pwd)/target/release/mac-cpu-monitord"
